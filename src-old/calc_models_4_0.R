...
calc_and_save_models <- function(clean_corpus, 
                                 model_id, use_small_data, use_pct_data, 
                                 do_mem_size_calc = FALSE) {

  cat('computing models [', model_id, ']\n')
  
  # key for cache file
  model_id <- paste0('model-4.', 
                     ifelse(use_small_data, 'small', 
                            format(use_pct_data, decimal.mark = '_')))
  ...
  dt.1 <- dt.1[, list(count = .N), by = word]       # word count 
  dt.1[, freq := count / sum(count)]                # word frequency
  dt.1[, id_word := seq(1, nrow(dt.1))]             # id for each word
  setindex(dt.1, word)                              # index on word
  setindex(dt.1, id_word)                           # index in id_word
  
  cat('    saving to cache\n')
  save_to_cache(dt.1, model_id, '1-gram')           # save in cache
  ...
  dt.n[, root := sapply(ngram, function(x) {paste(x[1:(length(x) - 1)], collapse = "_")})]
  ...
  cat('    saving to cache\n')
  save_to_cache(dt.n, model_id, 'n-gram')
